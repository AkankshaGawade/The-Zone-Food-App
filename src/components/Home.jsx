...
import { Delivery, HeroBg, heroBg } from "../assets";
    ...
    <motion.div className="w-full grid grid-cols-1 md:grid-cols-2 gap-4">
      <div className="flex flex-col items-start justify-start gap-6">
        ...
        <p className="text-[40px] text-headingColor md:text-[72px] font-sans font-extrabold tracking-wider">
          Food Cravings? We'Ve Got You Covered!
        </p>
        <p className="text-textColor text-lg">
          Food delivery is a home delivery service in which a store, restaurant,
          or third-party app delivers food to consumers, whenever they ask for
          it.
        </p>
        <motion.button
          {...buttonClick}
          className="bg-gradient-to-bl from-orange-400 to-orange-600 px-4 py-2 rounded-xl text-black font-semibold"
        >
          Order Now
        </motion.button>
      </div>
          ...
          {randomData &&
            randomData.map((data, i) => (
              <motion.div
                key={i}
                {...staggerFadeInOut(i)}
                className="w-32 h-36 md:h-auto md:w-190 p-4 bg-slate-200 backdrop-blur-md rounded-3xl flex flex-col items-center justify-center drop-shadow-lg"
              >
                <img
                  src={data.imageURL}
