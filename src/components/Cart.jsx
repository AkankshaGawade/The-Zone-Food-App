...
import { baseURL, getAllCartItems, incrementQuantity } from "../api";
...
const Cart = () => {
  ...
  return (
    <motion.div
      {...slideIn}
      className="fixed z-50 top-0 right-0 w-850 md:w-500 bg-slate-50 backdrop-blur-md shadow-md h-screen"
    >
      ...
      <div className="flex-1 flex flex-col items-start justify-start rounded-t-3xl bg-zinc-900 h-full py-6 gap-3 relative">
        {cart && cart?.length > 0 ? (
          <>
            <div className="flex flex-col w-full items-start justify-start gap-3 h-[60%] overflow-y-scroll scrollbar-none px-4">
              {cart &&
                cart?.length > 0 &&
                cart?.map((item, i) => (
            ...
            <div className="bg-zinc-800 rounded-t-[40px] w-full h-[25%] flex flex-col items-center justify-center px-4 py-6 gap-10">
              ...
              className="bg-orange-500 w-[70%]  px-4 text-xl text-headingColor font-semibold hover:bg-orange-500 drop-shadow-md rounded-2xl"
...
export const CartItemcard = ({ index, data }) => {
  ...
  const decrementCart = (productId) => {
    ...
    incrementQuantity(user?.user_id, productId, "decrement").then((data) => {
  ...
  const incrementCart = (productId) => {
    dispatch(alertSuccess("Updated the cart"));
    incrementQuantity(user?.user_id, productId, "increment").then((data) => {
      getAllCartItems(user?.user_id).then((items) => {
        dispatch(setCartItems(items));
        dispatch(alertNull());
      });
